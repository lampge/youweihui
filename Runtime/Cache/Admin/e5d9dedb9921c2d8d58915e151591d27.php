...
<div class="tab-wrap">
    <ul class="tab-nav nav">
	    ...
	    <li class="fr">
		    <select name="group">
			    <?php if(is_array($auth_group)): $i = 0; $__LIST__ = $auth_group;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo U('AuthManager/site',array('group_id'=>$vo['id'],'group_name'=>$vo['title']));?>" <?php if(($vo['id']) == $this_group['id']): ?>selected<?php endif; ?> ><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		    </select>
	    </li>
    </ul>
    <!-- 表格列表 -->
    <div class="tb-unit posr">
        <form class="save-category" action="<?php echo U('AuthManager/addToSite');?>" method="post" enctype="application/x-www-form-urlencoded">
            ...
            <div class="category auth-category">
                ...
                <?php $_result=C('SITE_LIST');if(is_array($_result)): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><dl class="cate-item">
                    <dt class="cf">
                    <div class="fold"><i></i><?php echo ($key); ?></div>
                    <label>
                        <div class="order"> <input class="cate_id" type="checkbox" name="sid[]" value="<?php echo ($key); ?>"></div>
                        <div class="name">
