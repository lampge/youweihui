...
com = {
    /*分页跳转*/
    change_page:function(type,page,url){
        if(com.isInt(page,1)){
            var param = '';
            var lin_type = '';
            switch(type){
                case 'channel_3':
                    ...
                    var orderby = $("#js_tag_con").find(".on").attr("htmlfor");
                    ...
                    var daynum = $("#daynum").find(".on_color").attr("htmlfor");
                    var class_4 = $('#class_4').find('.on_color').attr('htmlfor');
                    if(typeof(daynum)  =='undefined'){daynum='';}
                    var price_num = $("#price_num").find(".on_color").attr("htmlfor");
                    if(typeof(price_num)  =='undefined'){price_num='';}
                    var typeid2 = $("#typeid2").find(".on_color").attr("htmlfor");
                    if(typeof(typeid2)  =='undefined'){typeid2='';}
                    param = "?class_id="+class_id+ "&orderby="+orderby+ "&updown="+updown+"&daynum="+daynum+ "&price_num="+price_num+ "&typeid2="+typeid2+'&class_4='+class_4;
                    break;
                case 'channel_list':
    ...
    date_type:function(type_id,date,price,price_d,price_child_d,typename,stock){
       ...
       $('#child_price').val(price_child_d);
        ...
        if((uid > 0 && sale_member == 1) || sale_member==0){
            ...
            $('#adult_price').val(sale_price);
        }else{
            $('#price_cncn').html(price_d);
            $('#adult_price').val(price_d);
        }
