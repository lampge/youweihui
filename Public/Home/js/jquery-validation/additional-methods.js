...
jQuery.validator.addMethod("dateDiff", function(value, element) {
       ...
       var myDate=$('#addtime').val();
       var curDate=$('#today').val();
       var dt1 = new Date(Date.parse(curDate.replace(/-/g, '/')));
       var dt2 = new Date(Date.parse(myDate.replace(/-/g, '/')));
       var diff=Math.round((dt2.getTime() - dt1.getTime()) /dt);
       var judge=diff>0?false:true;
       return this.optional(element) || judge;
}, "下单日期不能晚于今天");
jQuery.validator.addMethod("less_than", function(value, element, params) {
	    return this.optional(element) || parseInt(value,10) <= parseInt('0'+params.val(),10);
	}, jQuery.validator.format("不能比{0}大或者相等"));
/**
