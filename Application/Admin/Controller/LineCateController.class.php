 ...
 */
class LineCateController extends AdminController {
    ...
    public function index(){
        $tree = D('LineCate')->getTree(0,'id,title,sort,pid,display,status');
        $this->assign('tree', $tree);
        C('_PRODUCT_CATE_TREE_', true); //标记系统获取分类树模板
        $this->assign('meta_title', '线路分类管理');
        $this->display();
    }
    ...
    public function tree($tree = null){
        C('_PRODUCT_CATE_TREE_') || $this->_empty();
        $this->assign('tree', $tree);
        $this->display('tree');
    }
    ...
    public function remove(){
        ...
        $child = M('LineCate')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
    ...
    public function operate($type = 'move'){
        ...
        $list = M('LineCate')->where($map)->field('id,pid,title')->select();
        ...
        if(strcmp($type, 'move') == 0){
        	...
        	$pid = M('LineCate')->getFieldById($from, 'pid');
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('LineCate')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
