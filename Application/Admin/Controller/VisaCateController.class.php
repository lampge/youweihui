 ...
 */
class VisaCateController extends AdminController {
    ...
    public function index(){
        $tree = D('VisaCate')->getTree(0,'id,title,sort,pid,display,status');
        $this->assign('tree', $tree);
        C('_PRODUCT_CATE_TREE_', true); //标记系统获取分类树模板
        $this->assign('meta_title', '签证分类管理');
        $this->display();
    }
    ...
    public function remove(){
        ...
        $child = M('VisaCate')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
    ...
    public function operate($type = 'move'){
        ...
        $list = M('VisaCate')->where($map)->field('id,pid,title')->select();
        ...
        if(strcmp($type, 'move') == 0){
        	...
        	$pid = M('VisaCate')->getFieldById($from, 'pid');
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('VisaCate')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
