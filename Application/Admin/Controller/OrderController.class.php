...
class OrderController extends AdminController {
    ...
    public function remove(){
        ...
        $child = M('ProductCate')->where(array('pid'=>$cate_id))->field('id')->select();
        if(!empty($child)){
        ...
        //删除该分类信息
        $res = M('ProductCate')->delete($cate_id);
        if($res !== false){
            //记录行为
    ...
    public function operate($type = 'move'){
        ...
        $list = M('ProductCate')->where($map)->field('id,pid,title')->select();
        ...
        if(strcmp($type, 'move') == 0){
        	...
        	$pid = M('ProductCate')->getFieldById($from, 'pid');
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('ProductCate')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
